...
public partial struct PlayerAttackSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var (expirationTimeStamp, attackData, transform) in SystemAPI.Query<RefRW<PlayerCooldownExpirationTimestamp>, PlayerAttackData, LocalTransform>())
        {
            ...
            var overlappedHits = new NativeList<int>(state.WorldUpdateAllocator);
            if (!physicsWorldSingleton.OverlapAabb(aabbInput, ref overlappedHits))
            {
                continue;
            }
            ...
            foreach (var item in overlappedHits)
            {
                var currentEnemyPosition = physicsWorldSingleton.Bodies[item].WorldFromBody.pos;
                var distanceToPlayerSq = math.distancesq(spawnPosition.xy, currentEnemyPosition.x);

                if (distanceToPlayerSq < maxDistanceSq)
                {
                    maxDetectPosition = distanceToPlayerSq;
...
public partial struct UpdateGemUISystem : ISystem
{
    public void OnUpdate(ref SystemState state)
    {
        foreach (var (gemCount, shouldUpdateUI) in SystemAPI.Query<GemCollectedCount, EnabledRefRW<UpdateGemUIFlag>>())
        {
